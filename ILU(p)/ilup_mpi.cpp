...
SparseMatrix<double>ILUp(const SparseMatrix<double>& matrix, int n, int p) {
    ...
    if (task_id == 0) {
        ...
        cur_matr.column_ind = colInd_tmp;
        cur_matr.row_ind = rowPtr_tmp;
        cur_matr.val = values_tmp;
        cur_matr.rows_count = block_size;
        cur_matr.columns_count = n;
